 ...
 * Symbols also start a new word; the ones in `S` are kept (see `words()`). `Run` holds the
 * kept symbols of the current run of symbols, or `false` outside of one.
 */
type Words<T extends string, S extends string, W extends string[] = [], Word extends string = '', Prev extends string = '', Run extends string | false = false> =
  T extends `${infer C}${infer Rest}`
    ? C extends Separator | SymbolChar
      ? C extends Separator
        ? Words<Rest, S, Push<W, `${Word}${RunText<Run>}`>, '', '', false>
        : Words<Rest, S, W, Word, Prev, `${RunText<Run>}${C extends S ? C : ''}`>
      : Run extends string
        ? Words<Rest, S, Push<W, Word>, `${Run}${C}`, C, false>
        : IsUpper<C> extends true
          ? Prev extends ''
            ? Words<Rest, S, W, `${Word}${C}`, C>
            : IsUpper<Prev> extends false
              ? Words<Rest, S, Push<W, Word>, C, C>
...
type ConvertKey<K extends string, C extends Case, S extends string> =
  ...
  C extends 'Dash' ? Join<Words<K, S>, '-'> :
  C extends 'UpperSnake' ? Uppercase<Join<Words<K, S>, '_'>> :
  C extends 'UpperDash' ? Uppercase<Join<Words<K, S>, '-'>> :
  C extends 'Train' ? Join<CapitalizeAll<Words<K, S>>, '-'> :
  C extends 'Dot' ? Join<Words<K, S>, '.'> :
